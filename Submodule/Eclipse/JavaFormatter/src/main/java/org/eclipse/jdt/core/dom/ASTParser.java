...
public class ASTParser {
	...
	private ASTNode internalCreateAST(IProgressMonitor monitor) {
		return internalCreateASTCached(monitor);
	}

	private ASTNode internalCreateASTCached(IProgressMonitor monitor) {
		boolean needToResolveBindings = false;
		switch (this.astKind) {
		...
		case K_COMPILATION_UNIT:
			try {
				...
				sourceUnit = new org.eclipse.jdt.internal.compiler.batch.CompilationUnit(this.rawSource, "", //$NON-NLS-1$
						this.unitName == null ? "" : this.unitName, null);
				CompilationUnit result = this.unitResolver.toCompilationUnit(sourceUnit);
				return result;
			} finally {
	...
	private ASTNode internalCreateASTForKind() {
		...
		ast.scanner.previewEnabled = JavaCore.ENABLED
				.equals(this.compilerOptions.get(JavaCore.COMPILER_PB_ENABLE_PREVIEW_FEATURES));
	...
	/*	public void setSource(ITypeRoot source, int astLevel) {
			...
			this.rawSource = null;
	//		if (source != null) {
	//			null = source.getJavaProject();
	//			Map<String, String> options = null.getOptions(true);
	//			options.remove(JavaCore.COMPILER_TASK_TAGS); // no need to parse task tags
	//			this.compilerOptions = options;
