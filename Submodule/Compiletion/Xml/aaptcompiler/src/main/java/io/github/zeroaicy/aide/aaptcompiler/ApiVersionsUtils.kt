...
class ApiVersionsUtils private constructor(val platform: File) {

    private var versions: ApiVersions?

    init {
        versions = readApiVersions(platform)
    }

    fun getApiVersion(): ApiVersions? {
        return versions
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun getInstance(platform: File): ApiVersionsUtils {
            return ApiVersionsUtils(platform)
        }
    }
    ...
    private fun readApiVersions(platform: File): ApiVersions? {
        ...
        Log.i(
            javaClass::getSimpleName.toString(),
            "Creating API versions table for platform dir: $platform"
        )
