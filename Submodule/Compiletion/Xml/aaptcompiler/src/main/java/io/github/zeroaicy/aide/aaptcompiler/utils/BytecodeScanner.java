...
/**
 * Scans jar files and saves all the class files that extends {@link View} and has the appropriate
 * constructors to be inflated in XML.
 */
public class BytecodeScanner {
  ...
  private static File getAndroidJar() {
    var androidJar = new File(ActivityThread.currentApplication().getNoBackupFilesDir(),".aide/android.jar");
    return new File(PreferenceManager.getDefaultSharedPreferences(ActivityThread.currentApplication()).getString("user_androidjar", androidJar.getAbsolutePath()));
  }
  ...
  public static void iterate(
      JarFile jarFile, Predicate<String> nameFilter, Consumer<JarEntry> consumer) {
